...
void main()
{
	...
	// Diffuse lighting
	diff = max(dot(normal, lightdir), 0.0); // Calculate the diffuse lighting

	// Specular lighting
	vec3 viewPos = vec3(0.0, 0.0, 0.0); // Position of the camera
	vec3 viewDir = normalize(viewPos - fragPos); // Direction from fragment to camera
	vec3 reflectDir = reflect(-lightdir, normal); // Reflect the light direction around the normal
	spec = pow(max(dot(viewDir, reflectDir), 0.0), phongExp); // Specular calculation
